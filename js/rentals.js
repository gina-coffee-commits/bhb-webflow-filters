(function () {
        ...
        opt.addEventListener('click', function (e) {
          ...
          if (val === 'Any') {
            ...
            for (var k = 0; k < opts.length; k++) {
              if (opts[k].classList.contains('is-active')) { hasActive = true; break; }
            }
            if (!hasActive && anyOpt) anyOpt.classList.add('is-active');
          }
          var selected = [];
          for (var k = 0; k < opts.length; k++) {
            if (opts[k].classList.contains('is-active') && opts[k].dataset.value !== 'Any') {
              selected.push(opts[k].dataset.value);
            }
          }
          if (trigText) trigText.textContent = selected.length ? selected.join(', ') : 'Any';
  ...
  function initPricePanel() {
    ...
    if (minTextEl && minTextEl.tagName !== 'INPUT') {
      ...
      minInput.type = 'text'; minInput.id = 'pwMinText';
      minInput.className = minTextEl.className; minInput.inputMode = 'numeric';
    ...
    if (maxTextEl && maxTextEl.tagName !== 'INPUT') {
      ...
      maxInput.type = 'text'; maxInput.id = 'pwMaxText';
      maxInput.className = maxTextEl.className; maxInput.inputMode = 'numeric';
