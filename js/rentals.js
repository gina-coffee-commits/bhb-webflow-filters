(function () {
  ...
  function cacheEls() {
    el = {
      keywordInput: document.querySelector('.keyword-input') || (function () {
        var fields = document.querySelectorAll('.rent-filter_top .filter-field');
    ...
    // Identify filter fields by their option values
    ...
    for (var i = 0; i < fields.length; i++) {
      ...
      var vals = [];
      for (var j = 0; j < opts.length; j++) vals.push(opts[j].dataset.value || '');
  ...
  function collectState() {
    ...
    if (el.bedsField) {
      ...
      for (var i = 0; i < actives.length; i++) {
        if (actives[i].dataset.value !== 'Any') state.bedrooms.push(actives[i].dataset.value);
      }
    }
    if (el.currField) {
  ...
  function passesPrice(d, card) {
    ...
    var txt     = priceEl ? priceEl.textContent.trim() : '';
  ...
  function injectBackToTop() {
    ...
    btt.addEventListener('click', function () {
      var target = document.querySelector('.rent-filter_form-block') || document.body;
