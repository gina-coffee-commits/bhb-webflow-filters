(function () {
  ...
  // ─── Config ──────────────────────────────────────────────────────────────────
  var CFG = { GRID_ID: 'lands-wrapper', CARD_SEL: '.w-dyn-item', STEP: 9 };
  ...
  // ─── Price Chip Presets (price per are) ──────────────────────────────────────
  var CHIP_PRESETS = {
    IDR: [
  ...
  var slider = { base: { min: 0, max: 5000000 }, active: { min: 0, max: 5000000 }, minRatio: 0, maxRatio: 1 };
  var el = {}, locUI = {};
  ...
  function getPricePerAre(inner) {
    ...
    // Fallback: total price divided by land size
  ...
  function cacheEls() {
    ...
    for (var i = 0; i < fields.length; i++) {
      ...
      } else if (labelTxt.indexOf('land size') > -1) {
  ...
  function initSingle(field, onPick) {
    ...
    trigger.addEventListener('click', function (e) { e.stopPropagation(); toggleDrop(dd); });
    ...
    for (var i = 0; i < opts.length; i++) {
      (function (opt) {
        opt.addEventListener('click', function (e) {
          ...
          if (f2) { var t2 = f2.querySelector('.filter-trigger'); if (t2) t2.classList.remove('is-active'); }
    ...
    for (var i = 0; i < drops.length; i++) {
      ...
      (function (drop) {
        btn.addEventListener('click', function (e) {
          ...
          if (field) { var trig = field.querySelector('.filter-trigger,.price-trigger'); if (trig) trig.classList.remove('is-active'); }
        });
      })(drops[i]);
    }
  }
  ...
  function bindEvents() {
    ...
    if (el.keywordInput) {
      ...
      el.keywordInput.addEventListener('input', function () {
        ...
        kwTimer = setTimeout(function () { state.keyword = el.keywordInput.value.trim(); applyFilters(); }, 300);
      });
    }

    if (el.btnClear) el.btnClear.addEventListener('click', clearAll);
    if (el.btnSearch) el.btnSearch.addEventListener('click', function (e) {
      e.preventDefault();
      applyFilters();
      closeAll();
      if (locDropOpen) openLocDrop(false);
    ...
    // Price trigger — handle .pricing-trigger-wrapper nesting
    ...
    if (el.locDropdown) el.locDropdown.addEventListener('click', function (e) { e.stopPropagation(); });
  ...
  function passesLandSize(d) {
    ...
    var s = d.size;
    var lbl = state.landSize.toLowerCase().replace(/[^\w\s]/g, '').trim();
    if (lbl.indexOf('under 200') > -1) return s < 200;
    if (lbl.indexOf('200') > -1 && lbl.indexOf('500') > -1) return s >= 200 && s <= 500;
    if (lbl.indexOf('500') > -1 && lbl.indexOf('1000') > -1) return s >= 500 && s <= 1000;
    if (lbl.indexOf('1000') > -1) return s > 1000;
    return true;
  }
  ...
  function passesLease(card) {
    ...
    if (lbl.indexOf('10') > -1 && lbl.indexOf('20') > -1) return years >= 10 && years <= 20;
    if (lbl.indexOf('20') > -1 && lbl.indexOf('25') > -1) return years >= 20 && years <= 25;
    if (lbl.indexOf('25') > -1 && lbl.indexOf('30') > -1) return years >= 25 && years <= 30;
    if (lbl.indexOf('30+') > -1) return years > 30;
    return true;
  }
  ...
  function setCurrency(currency) {
    ...
    if (window.debugCurrency && typeof window.debugCurrency.setCurrency === 'function') window.debugCurrency.setCurrency(c);
  ...
  function computeBaseBounds() {
    ...
    for (var i = 0; i < allCards.length; i++) {
      var inner = allCards[i].querySelector('.listings_card-wrapper') || allCards[i];
      var price = getPricePerAre(inner);
      if (!price || !isFinite(price)) continue;
      var idr = convertAmount(price, (inner.dataset.currency || 'IDR').toUpperCase(), 'IDR');
      if (idr < lo) lo = idr;
      if (idr > hi) hi = idr;
    }
  ...
  function fixSliderDOM() {
    ...
    if (track && fill && track.contains(fill)) { track.removeChild(fill); track.parentNode.insertBefore(fill, track.nextSibling); }
  ...
  function updateSliderForCurrency(currency) {
    ...
    if (el.priceTrigText) {
      el.priceTrigText.textContent = (slider.minRatio <= 0 && slider.maxRatio >= 1) ? 'Price Range' : (sym + short(minV) + ' – ' + sym + short(maxV));
    }
  }
  ...
  function initPricePanel() {
    ...
    if (minTextEl && minTextEl.tagName !== 'INPUT') {
      ...
      minInput.type = 'text'; minInput.id = 'pwMinText'; minInput.className = minTextEl.className; minInput.inputMode = 'numeric';
    ...
    if (maxTextEl && maxTextEl.tagName !== 'INPUT') {
      ...
      maxInput.type = 'text'; maxInput.id = 'pwMaxText'; maxInput.className = maxTextEl.className; maxInput.inputMode = 'numeric';
    ...
    function valToRatio(v) { return (v - slider.active.min) / (slider.active.max - slider.active.min || 1); }
    ...
    function render() {
      ...
      for (var i = 0; i < chips.length; i++) {
        chips[i].classList.toggle('is-active', Math.abs(Number(chips[i].dataset.min) - minV) < 1000 && Math.abs(Number(chips[i].dataset.max) - maxV) < 1000);
      }
    }

    var THUMB_SIZE = 20, dragging = null, dragTimer = null;
    ...
    if (!thumbMin) { thumbMin = document.createElement('div'); thumbMin.className = 'pw-thumb pw-thumb-min'; sw.appendChild(thumbMin); }
    ...
    function startDrag(which, e) {
      ...
      if (e.type === 'mousedown') { document.addEventListener('mousemove', onMouseMove); document.addEventListener('mouseup', onUp); }
      else { document.addEventListener('touchmove', onTouchMove, { passive: false }); document.addEventListener('touchend', onUp); }
    }
    ...
    if (trackEl) {
      trackEl.style.pointerEvents = 'auto'; trackEl.style.cursor = 'pointer';
    ...
    function bindTextInput(inputEl, which) {
      ...
      inputEl.addEventListener('change', function () {
        ...
        fullRender(); applyFilters();
      });
      inputEl.addEventListener('keydown', function (e) { if (e.key === 'Enter') inputEl.dispatchEvent(new Event('change')); });
    }
  ...
  function buildAreas() {
    ...
    for (var a = 0; a < AREA_RULES.length; a++) {
      ...
      var children = cmsLocs.filter(function (loc) { var hit = rule.keys.indexOf(loc) > -1; if (hit) used[loc] = true; return hit; });
  ...
  function mountLocUI() {
    ...
    if (lsd && lsd.tagName !== 'INPUT') {
      var ri = document.createElement('input'); ri.type = 'text'; ri.placeholder = 'Search...'; ri.className = 'location-search-input';
  ...
  function renderLocLists() {
    ...
    locUI.treeScroll.innerHTML = ''; locUI.pillScroll.innerHTML = '';
    for (var a = 0; a < areas.length; a++) {
      ...
      var childWrap = document.createElement('div'); childWrap.className = isOpen ? 'children open' : 'children';
      (function (cw) { parent.addEventListener('click', function () { cw.classList.toggle('open'); }); })(childWrap);
      item.appendChild(parent);
      var inner = document.createElement('div'); inner.className = 'children-inner'; inner.innerHTML = '<div class="branch"></div>';
      ...
      for (var k = 0; k < visKids.length; k++) {
        (function (loc) {
          ...
          row.innerHTML = '<div class="mini-pin-box"><img src="' + PIN_URL + '" class="pin-icon-sm" alt=""></div><span>' + (labelByNorm[loc] || loc) + '</span>';
      ...
      var pill = document.createElement('div'); pill.className = 'pill' + (areaActive ? ' is-active' : ''); pill.textContent = area.label;
      (function (aId) { pill.addEventListener('click', function () { toggleArea(aId); }); })(area.id);
      locUI.pillScroll.appendChild(pill);
    }
    updateDraftInfo();
  }

  function toggleLoc(loc) { var idx = draftLocs.indexOf(loc); if (idx > -1) draftLocs.splice(idx, 1); else draftLocs.push(loc); renderLocLists(); syncMapWith(draftLocs); }
  ...
  function loadMapSDK(cb) {
    ...
    if (!document.querySelector('link[data-mt-css]')) {
      var css = document.createElement('link'); css.rel = 'stylesheet'; css.href = 'https://cdn.maptiler.com/maptiler-sdk-js/v3.10.2/maptiler-sdk.css'; css.setAttribute('data-mt-css', '1'); document.head.appendChild(css);
    }
    var ex = document.querySelector('script[data-mt-js]');
    if (ex) return ex.addEventListener('load', cb, { once: true });
    var js = document.createElement('script'); js.src = 'https://cdn.maptiler.com/maptiler-sdk-js/v3.10.2/maptiler-sdk.umd.min.js'; js.async = true; js.setAttribute('data-mt-js', '1'); js.onload = cb; document.head.appendChild(js);
  }
  function initMap() {
    if (map) return;
  ...
  function makePin(label) {
    ...
    var img = document.createElement('img'); img.src = PIN_URL; img.alt = 'marker'; img.style.cssText = 'width:26px;height:26px;object-fit:contain;filter:drop-shadow(0 2px 6px rgba(0,0,0,.25))';
  ...
  function syncMapWith(locations) {
    ...
    for (var i = 0; i < locations.length; i++) {
      var p = LOC_COORDS[locations[i]]; if (!p) continue;
      var key = p[0] + ',' + p[1]; if (!seen[key]) { seen[key] = true; pts.push({ p: p, loc: locations[i] }); }
    }
    if (!pts.length) { map.flyTo({ center: [115.1889, -8.4095], zoom: 9.3, duration: 450 }); return; }
    for (var i = 0; i < pts.length; i++) {
      var label = (labelByNorm[pts[i].loc] || pts[i].loc).replace(/\b\w/g, function (c) { return c.toUpperCase(); });
      markers.push(new maptilersdk.Marker({ element: makePin(label), anchor: 'bottom' }).setLngLat(pts[i].p).addTo(map));
    }
    if (pts.length === 1) { map.flyTo({ center: pts[0].p, zoom: 12.2, duration: 450 }); return; }
